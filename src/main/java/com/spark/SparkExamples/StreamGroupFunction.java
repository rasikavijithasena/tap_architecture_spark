...
public class StreamGroupFunction{
    ...
    public static void readDataStream(SparkSession spark, JavaStreamingContext ssc) throws InterruptedException {
        ...
        JavaDStream<Transaction> tranStream = logdata
                .window(new Duration(10000), new Duration(2000))
                .map(line -> {
 ...
 *
            ...
            Dataset<String> dates = df2.map((MapFunction<Row, String>) row -> row.<String>getAs("time_stamp").substring(0,10), Encoders.STRING());
            JavaRDD<String> ds = dates.toJavaRDD();
            JavaPairRDD<String, Integer> count = ds.mapToPair(word -> {return new Tuple2<>(word, 1);}).reduceByKey((a, b) -> a + b);
 ...
 //           spark.sql("INSERT INTO TABLE trans_hive_spark SELECT * FROM allData");


            df2_for_save.createOrReplaceTempView("date_summary");
            spark.sql("INSERT INTO TABLE stream_summary SELECT * FROM date_summary");
            Dataset<Row> monthly = spark.sql("SELECT time_stamp FROM stream_summary");

            Dataset<String> months = monthly.map((MapFunction<Row, String>) row -> row.<String>getAs("time_stamp").substring(0,7), Encoders.STRING());
            JavaRDD<String> ds1 = months.toJavaRDD();

            JavaPairRDD<String, Integer> count1 = ds1.mapToPair(word -> {return new Tuple2<>(word, 1);}).reduceByKey((a, b) -> a + b);

            System.out.println("Monthly summary");
            List<Tuple2<String, Integer>> output1 = count1.collect();
