...
/**         tranStream.foreachRDD(rdd->{
 ...
 if(tranStream!=null) {
 File f = new File("streaming_kafka.parquet");
 if (f.exists() || f.isDirectory()) {
 counts.write().mode(SaveMode.Append)
 .format("parquet")
 .save("streaming_kafka.parquet");
 ...
 Dataset<Row> s2 = spark.read().parquet("streaming_kafka.parquet");
 ...
 **/
    ...
    public static void readKafka(SparkSession spark) {

        Dataset<Row> parquetFileDF = spark.read().parquet("streaming_kafka.parquet");
