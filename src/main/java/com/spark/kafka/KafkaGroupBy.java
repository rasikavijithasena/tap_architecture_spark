...
public class KafkaGroupBy {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        SparkConf conf = new SparkConf().setAppName("com.spark.kafka.KafkaGroupBy").setMaster("local[*]");
...
//            Dataset<Row> ds2 = spark.createDataFrame(JavaPairRDD.toRDD(x),Encoders.tuple(Encoders.bean(sum.class), Encoders.INT())).toDF("key","value");
            ...
            for (Tuple2<?,?> tuple : output) {
                ...
                dailySummaryList.add(s);

            }

            if(dailySummaryList.size() != 0){

                Dataset<Row> dbDataset = spark.createDataFrame(dailySummaryList, DailySummary.class);
                dbDataset.show();
                ...
                if(dailySummary.count() != 0) {
                    ...
                    Dataset<DailySummary> dailyObjects = dailySummary.as(Encoders.bean(DailySummary.class));

                    List<DailySummary> dataList = dailyObjects.takeAsList(100000);


                    List<DailySummary> insertList = new ArrayList<DailySummary>();


                    outloop:
                    for (DailySummary dailySummaryNew : dailySummaryList){

                        int counter = 0;

                        inloop:
                        for (DailySummary db : dataList){


                            if (dailySummaryNew.getDate().equals(db.getDate()) && dailySummaryNew.getSp_id().equals(db.getSp_id())){
                                String value = String.valueOf(Integer.parseInt((dailySummaryNew.getNum())) + Integer.parseInt((db.getNum())));
                                ...
                                //insert quary
                                //insertList.add(dailySummaryNew);
                    ...
                    Dataset<Row> updateDataset = spark.createDataFrame(dataList, DailySummary.class);
                    ...
                    if(insertList.size() != 0) {
                        ...
                        Dataset<Row> insertDataset = spark.createDataFrame(insertList, DailySummary.class);
